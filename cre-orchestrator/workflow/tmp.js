...
`), { name: "AbiEncodingLengthMismatchError" });
  ...
  AbiEventSignatureEmptyTopicsError = class AbiEventSignatureEmptyTopicsError2 extends BaseError {
  ...
  AbiEventSignatureNotFoundError = class AbiEventSignatureNotFoundError2 extends BaseError {
...
`), {
  ...
  DecodeLogDataMismatch = class DecodeLogDataMismatch2 extends BaseError {
...
`), {
  ...
  DecodeLogTopicsMismatch = class DecodeLogTopicsMismatch2 extends BaseError {
...
init_toEventSelector();
var EVM_EXTRA_ARGS_V1_TAG = "0x97a657c9";
var VAULT_EVENTS_ABI = [
  {
    type: "event",
...
var EVENT_SELECTORS = [
  toEventSelector("DepositRequested(address,address,uint256,uint64)"),
  toEventSelector("WithdrawRequested(address,address,uint256,uint64)"),
  toEventSelector("WithdrawExecutionRequested(address,address,uint256,uint64)")
];
var buildTopics = () => [
...
var buildExtraArgs = (extraArgsGasLimit) => {
  if (extraArgsGasLimit === null || extraArgsGasLimit === undefined) {
    return "0x";
  }
  const gasLimit = typeof extraArgsGasLimit === "string" ? BigInt(extraArgsGasLimit) : BigInt(extraArgsGasLimit);
  const encodedArgs = encodeAbiParameters([{ type: "uint256" }], [gasLimit]);
  return concatHex([EVM_EXTRA_ARGS_V1_TAG, encodedArgs]);
};
var encodeReceiverBytes = (receiver) => encodeAbiParameters([{ type: "address" }], [receiver]);
var encodeOperationData = (operation, params) => encodeAbiParameters([{ type: "string" }, { type: "bytes" }], [operation, params]);
var buildMessage = (params) => ({
...
var readErc20Balance = (runtime2, ctx, token, owner) => {
  const callData = encodeFunctionData({
    abi: ERC20_ABI,
    functionName: "balanceOf",
    args: [owner]
  });
  const response = ctx.evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: token,
      data: callData
...
var estimateFee = (runtime2, ctx, destinationChainSelector, message) => {
  const callData = encodeFunctionData({
    abi: ROUTER_ABI,
    functionName: "getFee",
...
var sendMessage = (runtime2, ctx, destinationChainSelector, message, writeGasLimit) => {
  const callData = encodeFunctionData({
    abi: VAULT_WRITE_ABI,
  ...
  const gasLimit = writeGasLimit === null || writeGasLimit === undefined ? undefined : typeof writeGasLimit === "string" ? writeGasLimit : writeGasLimit.toString();
  return ctx.evmClient.writeReport(runtime2, {
    receiver: hexToBytes(ctx.config.vaultAddress),
    report: report2,
    $report: true,
    gasConfig: gasLimit ? { gasLimit } : undefined
  }).result();
};
var preflightChecks = (runtime2, ctx, destinationChainSelector, message, preflight) => {
  let requiredFee = 0n;
  if (preflight.checkLink) {
    requiredFee = estimateFee(runtime2, ctx, destinationChainSelector, message);
    const linkBalance = readErc20Balance(runtime2, ctx, ctx.config.linkTokenAddress, ctx.config.vaultAddress);
    if (linkBalance < requiredFee) {
      runtime2.log(`[${ctx.config.name}] Insufficient LINK. Have ${linkBalance} need ${requiredFee}`);
      return false;
    }
  }
  if (preflight.checkToken && message.tokenAmounts.length > 0) {
    const { token, amount } = message.tokenAmounts[0];
    const tokenBalance = readErc20Balance(runtime2, ctx, token, ctx.config.vaultAddress);
    if (tokenBalance < amount) {
      runtime2.log(`[${ctx.config.name}] Insufficient token balance. Have ${tokenBalance} need ${amount}`);
      return false;
    }
  }
  return true;
};
var handleVaultLog = (runtime2, log, sourceCtx, chainBySelector, settings) => {
  if (log.removed) {
    runtime2.log(`[${sourceCtx.config.name}] Ignoring removed log ${bytesToHex(log.txHash)}`);
    return { status: "skipped", detail: "log_removed" };
  }
  let decoded;
  if (log.topics.length === 0) {
    runtime2.log(`[${sourceCtx.config.name}] Log has no topics; skipping decode.`);
  ...
  try {
    ...
    runtime2.log(`[${sourceCtx.config.name}] Failed to decode log: ${error.message}`);
    return { status: "error", detail: "decode_failed" };
  }
  if (!decoded) {
    runtime2.log(`[${sourceCtx.config.name}] Unsupported event payload`);
  ...
  try {
    if (decoded.eventName === "DepositRequested") {
      const { user, token, amount, targetChainSelector } = decoded.args;
      const destination = chainBySelector.get(targetChainSelector.toString());
      if (!destination) {
        runtime2.log(`[${sourceCtx.config.name}] Unknown destination selector ${targetChainSelector.toString()}`);
      ...
      if (!preflightChecks(runtime2, sourceCtx, destination.chainSelector, message, settings.preflight)) {
        return { status: "skipped", detail: "preflight_failed" };
      }
      const result = sendMessage(runtime2, sourceCtx, destination.chainSelector, message, settings.writeGasLimit);
      const txHash = result.txHash ? bytesToHex(result.txHash) : undefined;
      runtime2.log(`[${sourceCtx.config.name}] DepositRequested sent via CCIP. Tx status ${result.txStatus} ${txHash ? `txHash ${txHash}` : ""}`.trim());
      return {
        status: "sent",
    ...
    if (decoded.eventName === "WithdrawRequested") {
      ...
      const result = sendMessage(runtime2, sourceCtx, destination.chainSelector, message, settings.writeGasLimit);
      const txHash = result.txHash ? bytesToHex(result.txHash) : undefined;
      runtime2.log(`[${sourceCtx.config.name}] WithdrawRequested sent via CCIP. Tx status ${result.txStatus} ${txHash ? `txHash ${txHash}` : ""}`.trim());
      return {
        status: "sent",
    ...
    if (decoded.eventName === "WithdrawExecutionRequested") {
      ...
      const result = sendMessage(runtime2, sourceCtx, destination.chainSelector, message, settings.writeGasLimit);
      const txHash = result.txHash ? bytesToHex(result.txHash) : undefined;
      runtime2.log(`[${sourceCtx.config.name}] WithdrawExecutionRequested sent via CCIP. Tx status ${result.txStatus} ${txHash ? `txHash ${txHash}` : ""}`.trim());
      return {
        status: "sent",
    ...
    runtime2.log(`[${sourceCtx.config.name}] Handler error: ${error.message}`);
    return { status: "error", detail: "handler_exception" };
  }
  return { status: "skipped", detail: "no_handler_match" };
};
var createChainContext = (chain) => {
  ...
  if (!network248) {
    throw new Error(`Unsupported chain selector name: ${chain.chainSelectorName}`);
  }
  return {
    config: chain,
    chainSelector: network248.chainSelector.selector,
    evmClient: new cre.capabilities.EVMClient(network248.chainSelector.selector)
  };
};
var initWorkflow = (config) => {
  const chainContexts = config.chains.map(createChainContext);
  const chainBySelector = new Map(chainContexts.map((ctx) => [ctx.chainSelector.toString(), ctx]));
  return chainContexts.map((ctx) => handler(ctx.evmClient.logTrigger({
    addresses: [hexToBase64(ctx.config.vaultAddress)],
    topics: buildTopics(),
    confidence: "CONFIDENCE_LEVEL_SAFE"
  }), (runtime2, log) => handleVaultLog(runtime2, log, ctx, chainBySelector, {
